...
export const mutations = {
  ...
  UPDATE_FACILITY(state, updatedFacility) {
    const index = state.allFacilities.findIndex(
...
export const actions = {
  async fetchProperties({ commit, rootState }) {
    ...
    try {
      const res = await fetch(`${this.$config.baseUrl}/properties/`, {
        method: "GET",
        headers: {
  ...
  async fetchCategories({ commit }) {
    try {
      const response = await fetch(
        `${this.$config.baseUrl}/properties/categories/`
      );
      if (!response.ok) {
  ...
  async deleteCategory({ commit }, categoryId) {
    try {
      const response = await fetch(
        `${this.$config.baseUrl}/properties/categories/${categoryId}/`,
        {
          method: "DELETE",
        }
      );
  ...
  async updateCategory({ commit, rootState }, name) {
    console.log(name);
    const url = `${this.$config.baseUrl}/properties/categories/${name.name.id}/`;
    console.log(url);
  ...
  async fetchFacilities({ commit }) {
    try {
      const response = await fetch(
        `${this.$config.baseUrl}/properties/facilities/`
      );
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      } else {
  ...
  async deleteFacility({ commit }, facilityId) {
    try {
      const response = await fetch(
        `${this.$config.baseUrl}/properties/facilities/${facilityId}/`,
        {
          method: "DELETE",
        }
      );
